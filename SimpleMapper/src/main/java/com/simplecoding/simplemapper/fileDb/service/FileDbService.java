...
public class FileDbService {
    ...
    public void insert(FileDb fileDb) throws Exception {
        String uuid = UUID.randomUUID().toString();                           // 1) UUID 만들기(기본키): 자바에서 중복안되게 만들어주는 글자(랜덤)
        fileDb.setUuid(uuid);
        if (fileDb.getFileData().getSize() > 0) {                           // 첨부파일을 업로드하지 않으면 크기가 0임(즉 이미지를 업로드 했을때만)
            String downloadURL = commonUtil.generateUrl("fileDb", uuid); // 2) 업로드 파일이 있을때만 다운로드 URL 만들기
            fileDb.setFileUrl(downloadURL);
            commonUtil.saveFile(fileDb.getFileData(), uuid);                // 3) 업로드 파일이 있을때만 파일 만들기
        }
